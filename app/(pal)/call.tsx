...
// Safe constants for FaceDetector
...
export default function CallScreen() {
    ...
    useEffect(() => {
        if (type === 'video' && status === 'active' && !isCameraOff) {
            const interval = setInterval(() => {
                const now = Date.now();
                // Only show warning if we've seen a face before (proving it's working) 
                // and now it's gone. This avoids warnings on unsupported devices.
    ...
    useEffect(() => {
        console.log(`[Call] Initialized call with type: ${type}`);
        StatusBar.setBarStyle('light-content');

        // Request camera if video call
        if (type === 'video') {
            requestPermission();
        }

        // --- Simulated Call State Flow (Mocking Socket Events) ---
        console.log('[Call] Mock: Emitting video-call:initiate');

        // 1. Initial State: "Calling..." (already set at start)

        // 2. Transition to "Ringing..." after 1.5s
        const rxTimeout = setTimeout(() => {
            setStatus(current => {
                if (current === 'calling') {
                    console.log('[Call] Mock: State change: calling -> ringing (video-call:incoming)');
        ...
        // 3. Transition to "Connecting..." after 4.5s
        const cnTimeout = setTimeout(() => {
            setStatus(current => {
                if (current === 'ringing') {
                    console.log('[Call] Mock: State change: ringing -> connecting (video-call:accepted)');
                    return 'connecting';
                }
                return current;
            });
        ...
        // 4. Transition to "Active" (Connected) after 6s
        const acTimeout = setTimeout(() => {
            setStatus(current => {
                if (current === 'connecting') {
                    console.log('[Call] Mock: State change: connecting -> active (peer-connection:connected)');
        ...
        // 5. No Answer Safety Timeout (30s)
        const naTimeout = setTimeout(() => {
            setStatus(current => {
                if (current !== 'active' && current !== 'ended' && current !== 'rejected') {
                    console.log('[Call] Mock: State change: -> no-answer (timeout)');
                    return 'no-answer';
                }
                return current;
            });
        ...
        return () => {
            console.log('[Call] Cleanup: clearing timeouts');
        ...
        return (
            <View style={{ flex: 1, backgroundColor: '#000' }}>
                <CameraView
                    ...
                    facing="front"
                    // @ts-ignore - Prop existence depends on native module presence at runtime
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#0F172A' }}>
            ...
            <View className="absolute inset-0 items-center justify-center opacity-20">
                <Animated.View style={[pulseStyle]} className="w-[500px] h-[500px] rounded-full bg-emerald-500/20" />
            </View>

            <View style={{ paddingTop: insets.top + 100 }} className="items-center px-10">
                <Animated.View entering={FadeIn.delay(200)}>
                    <View className="relative">
                        <Animated.View style={[pulseStyle]} className="absolute inset-0 bg-emerald-500/30 rounded-[64px]" />
                        <View className="w-40 h-40 bg-slate-800 rounded-[64px] items-center justify-center border-2 border-emerald-500 overflow-hidden">
                            {partnerAvatar ? (
                ...
                <Animated.View entering={FadeInDown.delay(400)} className="items-center mt-12">
                    ...
                    <View className="flex-row items-center bg-white/5 px-6 py-3 rounded-full mt-6 border border-white/10">
                        {status === 'calling' && (
                            <ActivityIndicator size="small" color="#10B981" style={{ marginRight: 12 }} />
                        )}
                        {status === 'ringing' && (
                            <Ionicons name="notifications" size={16} color="#10B981" style={{ marginRight: 12 }} />
                        )}
                        {status === 'connecting' && (
                            <ActivityIndicator size="small" color="#3B82F6" style={{ marginRight: 12 }} />
                        )}
                        <Text className={`font-black uppercase tracking-[4px] text-xs ${status === 'active' ? 'text-white' : 'text-emerald-500'}`}>
